...
export default function TaskForm({ initial = {}, onClose, onSuccess }) {
  ...
  useEffect(() => {
    Promise.all([api.users.list(), api.squads.list()]).then(([u, s]) => {
      setUsers(Array.isArray(u?.data) ? u.data : []);
      setSquads(Array.isArray(s?.data) ? s.data : []);
    });
  }, []);
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.success(
        initial.id ? "Tarefa atualizada!" : "Tarefa criada com sucesso!",
        {
          duration: 3000,
        }
      );
              ...
              >
                ...
                {users.map((u) => (
                  <option key={u.id || u._id} value={u.id || u._id}>
                    {u.name || u.nome || u.email || `Usuário ${u.id || u._id}`}
                  </option>
                ))}
              </select>
            </label>
              ...
              >
                ...
                {squads.map((s) => (
                  <option key={s.id || s._id} value={s.id || s._id}>
                    {s.name || s.nome || `Squad ${s.id || s._id}`}
                  </option>
                ))}
              </select>
            </label>
